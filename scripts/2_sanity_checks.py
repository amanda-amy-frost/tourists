...
assert COUNTRY_ISO_CODES == sorted(
    pop.unique(subset='iso_code').get_column('iso_code')
)
assert COUNTRY_ISO_CODES == sorted(
    climate.unique(subset='iso_code').get_column('iso_code')
)
assert COUNTRY_ISO_CODES == sorted(
    tourists.unique(subset='iso_code').get_column('iso_code')
)
...
for iso_code in COUNTRY_ISO_CODES:
    ...
    assert df_by_country[iso_code].null_count().sum_horizontal().item() == 8
